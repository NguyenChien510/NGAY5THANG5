...
void Summary(LISTPACKAGES &l){
	NODEPACKAGE temp = l.head;NODEPACKAGE tempp = l.head;int demHC=0,demLC=0,demTK=0,demCL=0,demST=0,demNHS=0,demHV=0,demHS=0,option;string address;
	while(temp!=NULL){
	...
	cout << "Khong co buu kien nao o khu vuc nay!!"<<endl<<endl;
		...
		while(tempp!=NULL){
			...
			cout << "SDT           : " << tempp->data.sender_number<<endl;
			cout << "Ten nguoi nhan: " << tempp->data.receiver<<endl;cout << "SDT           : "<< tempp->data.receiver_number<<endl;
			...
			cout << "Shipper       : " << tempp->data.shipper<<endl;
...
string GetPassword() {
    ...
    cout << endl; // Xuong dong sau khi nhap xong
    return pass;
}
void loginoutput() {
    string user, pass;
